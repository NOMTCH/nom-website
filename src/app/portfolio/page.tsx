    ...
    <main className="min-h-screen bg-background pt-[120px] pb-32 relative text-foreground selection:bg-accent selection:text-white">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10 max-w-7xl">
        ...
        <header className="mb-20 mt-8">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-display font-black mb-6 text-foreground uppercase tracking-tight leading-none">
        ...
        <section className="mb-28">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-display font-black uppercase text-foreground">Service Categories</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {categoryCards.map((cat) => (
              <Link 
                key={cat.id}
                href={cat.href}
                className="group relative flex flex-col bg-surface border border-border/80 rounded-[2.5rem] overflow-hidden shadow-xl hover:shadow-2xl hover:border-accent hover:-translate-y-1.5 transition-all duration-500"
              >
                <div className="aspect-[16/10] w-full relative overflow-hidden bg-background">
                  ...
                  <div className="absolute top-4 right-4 w-10 h-10 rounded-2xl bg-surface/80 backdrop-blur-md border border-border/80 flex items-center justify-center text-foreground group-hover:bg-accent group-hover:text-white group-hover:border-accent transition-all duration-300 shadow-md">
                    <ArrowUpRight size={20} weight="bold" />
                  </div>
                </div>

                <div className="p-8 flex flex-col flex-1 relative z-10">
                  <h3 className="text-2xl font-display font-black text-foreground group-hover:text-accent transition-colors duration-300 uppercase tracking-tight mb-3">
                    {cat.title}
                  </h3>
                  <p className="text-sm font-medium text-muted line-clamp-2 leading-relaxed">
                    {cat.description}
                  </p>
                </div>
              </Link>
        ...
        {recentProjectItems.length > 0 && (
          <section className="border-t border-border/80 pt-20">
            <div className="mb-12">
              <span className="text-accent font-bold tracking-widest text-xs uppercase px-3 py-1 bg-accent/10 border border-accent/20 rounded-full inline-block mb-3">
                LIVE PROJECT SHOWCASE
              </span>
              <h2 className="text-3xl md:text-5xl font-display font-black uppercase text-foreground">Recent Featured Work</h2>
