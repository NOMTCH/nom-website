                ...
                <ul className="space-y-4">
                  {pkg.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-4">
                      <div className={`mt-1 shrink-0 p-1 border-2 ${pkg.is_popular ? 'border-accent bg-accent/20 text-accent' : 'border-black bg-black/5 text-black'}`}>
                        <Check weight="bold" size={12} />
                      </div>
                      <span className={`font-bold ${pkg.is_popular ? 'text-gray-300' : 'text-gray-800'}`}>
                        {feature}
                      </span>
                    </li>
                  ))}
